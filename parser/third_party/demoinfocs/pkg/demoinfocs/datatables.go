...
func (p *parser) bindBomb() {
	...
	scC4.OnEntityCreated(func(bombEntity st.Entity) {
		...
		bombEntity.Property("m_hOwnerEntity").OnUpdate(func(val st.PropertyValue) {
			var carrier *common.Player
			if handle, ok := optionalPropertyHandle(val); ok {
		...
		bombEntity.Property("m_bStartedArming").OnUpdate(func(val st.PropertyValue) {
			isArming, ok := optionalPropertyBool(val)
			if !ok {
				return
			}
...
func optionalPropertyBool(val st.PropertyValue) (bool, bool) {
	if val.Any == nil {
		return false, false
	}
...
func optionalPropertyHandle(val st.PropertyValue) (uint64, bool) {
	if val.Any == nil {
		return 0, false
	}
