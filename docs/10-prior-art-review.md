...
- User-provided Skybox smoke references demonstrate a stronger replay-reading hierarchy than a faint neutral field: dense light-grey coverage, restrained fine texture, a darker separating edge, and a large lifecycle arc remain legible at a glance. DemoRead should adapt that language from canonical center/side/timing data without copying Skybox artwork or its unavailable geometry-aware footprint.
