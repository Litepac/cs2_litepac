...
- Flashbang and HE detonation positions/ticks come from canonical utility events. Their Live bursts are screen-space cues normalized to the installed `0.25s` flash and `0.1s` HE-flare lifetimes, including the exact detonation tick; flash uses a broader soft flare for legibility while HE remains compact, and neither claims a world-space blast radius. Flashbang consequences are rendered separately from parser-owned per-player blind duration/end ticks, while HE damage labels come from parser-owned hurt events.
