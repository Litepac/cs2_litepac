...
- The installed flash detonation resource (`explosion_flashbang.vpcf_c`, VPK CRC `00bee8cceb`) emits one additive sprite with a `0.25s` lifetime. The HE's dedicated anamorphic flash child (`explosion_hegrenade_flash_ana.vpcf_c`, VPK CRC `00ea3a2b2c`) uses a `0.1s` lifetime, while separate HE children own smoke, debris, heat trails, and embers. DemoRead should therefore treat both map bursts as compact time-normalized event cues, never as blast-radius overlays. Flash impact belongs on parser-owned blinded-player windows, and HE impact belongs on parser-owned hurt events and damage values.
