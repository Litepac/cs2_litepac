# Third-Party Asset Notice

This repository contains extracted Counter-Strike 2 interface artwork under:

- `viewer/src/icons/cs2-equipment/`
- `viewer/src/icons/cs2-deathnotice/`

Those files are third-party game assets, not original DemoRead artwork. Counter-Strike and related Valve names and marks belong to their respective owners. No license to those third-party assets is granted by this repository.

Before a public/domain-facing release, replace these extracted assets with repo-owned equivalents or complete a separate rights review for their intended distribution and use.

The locally exported Source 2 map assets under ignored `public/maps/*/3d` paths are development inputs only. They are excluded from production builds and must not be treated as redistributable release assets.
