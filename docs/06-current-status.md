...
- Molotov/incendiary coverage is stronger: the parser samples demoinfocs active inferno fire-cell positions every four ticks. The viewer flattens the current sample into one bounded connected raster field, matching the installed Valve core/fill/outline composition without turning individual implementation cells into highlighted discs. Color and texture remain presentation, while the changing footprint, separated regions, growth, and extinction come from canonical samples; malformed oversized geometry falls back to the older non-spatial marker instead of allocating an unbounded texture.
- Flashbang and HE detonation positions/ticks come from canonical utility events. Their Live bursts are screen-space cues normalized to the installed `0.25s` flash and `0.1s` HE-flare lifetimes, including the exact detonation tick; neither claims a world-space blast radius. New parser artifacts bind each blind event to the exact flash projectile's stable utility id. Live uses that binding for an `0.8s` analytical hold of soft dashed origin-to-victim links, with intensity derived from recorded blind duration; the existing player ring continues for the exact blind window. HE damage labels remain parser-owned hurt-event truth. Older artifacts without a source utility id omit the links instead of guessing.
- Decoys use their parser-owned detonation point and exact active/expire window for the point marker and timer. The viewer does not invent the engine's synthetic weapon-shot cadence because canonical replay does not currently expose it.
...
- The fresh `1-0170...` Mirage demo has source SHA-256 `d6f00fe374872c4a1d3cdc94e1137300ffb92bf0cde77a1f6d1ae2eeb2b2e674` and one parser-owned A-site explosion at tick `95653`. Its exact hash is now locally allowlisted against that current Mirage resource; unrelated and historical Mirage demos still fall back to the non-spatial cue.
- A July 24 post-update Ancient demo with SHA-256 `3b03bdeaec2d4c15c04ce990bad4758b12737df4aaa935e60ee8ff90e8970b31` parses and validates as a 24-round, 64-tick replay with three explosion events. Its compatible installed Ancient resource has SHA-256 `CBFDB90169637D18ABDE834D2542CB0AB196F634F754908D3374A674EF7F8F74`, with baked propagation ranges `2875` for A and `3060` for B. Local ignored A/B masks are allowlisted only for that exact demo hash; the March Ancient fixture remains on the non-spatial fallback because applying the July map field to it would be historically wrong.
- `tools/export-all-source2-bomb-damage.ps1` now prepares ignored A/B masks for all ten supported defusal maps without overwriting existing demo allowlists. The current local inventory is complete, and exact July demo hashes now bind the current fields for Ancient, Cache, Dust II, Inferno, Mirage, and Nuke. The four newly supplied replays parse cleanly at 64 tick; Cache, Nuke, and Inferno contain two, two, and three explosions respectively, while Dust II contains eight plants but no explosion and therefore proves parser/map compatibility without exercising the spatial cue. Anubis, Overpass, Train, and Vertigo remain deliberately disabled until a fresh demo for each map is supplied; having the current map resource alone is not proof that a historical demo used that build.
...
- `tools/export-source2-bomb-damage.ps1` extracts the exact installed map resource and generates ignored, SHA-keyed A/B extent masks under `public/maps/<mapId>/bomb-damage/`. The manifest also allowlists explicit source-demo SHA-256 values so historical demos do not silently inherit the current map build's field. Compatible masks now encode normalized propagation cost as a pale-yellow core through orange to a red field edge, attack immediately, remain strongly readable through `1.4s`, and fade out by `3.6s`; this is presentation of the compiled field, not a damage scale or simulated arrival wave. Without a compatible mask the viewer shows only a non-spatial detonation cue. Generated Valve-derived data stays out of Git, and neither path claims exact damage, player outcome, or shockwave arrival time.
