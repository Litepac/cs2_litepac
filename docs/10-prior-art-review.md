...
- The current installed CS2 server/client binaries expose flash detonation, blind-kill threshold, spectator-opacity, and `cl_display_flashbang_values` controls, but no tunable gameplay flash radius or range. Keep the burst explicitly symbolic and make it responsive enough to read at full-map scale; use parser-bound links and blind rings as the exact non-circular truth instead of fitting another subtle circumference or field that competes with the obvious detonation cue.
