...
- The canonical `exploded` bomb event still provides a trustworthy tick and origin, but the replay does not own Valve's map-compiled, occlusion-aware shockwave field. The viewer therefore uses a bounded screen-space flash with broken leading/trailing event fronts and no longer draws guessed circular damage-radius bands. The cue communicates the explosion event, not shockwave reach or damage.
