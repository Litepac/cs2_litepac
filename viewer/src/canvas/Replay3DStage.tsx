...
type Replay3DCameraMode = "tactical" | "chase" | "pov" | "free";
...
const SHARED_3D_MODEL_RESOURCE_FLAG = "demoReadShared3DModelResource";
...
const TACTICAL_REVIEW_CAMERA_FOV = 52;
const POV_REVIEW_CAMERA_FOV = 74;
const USE_EXPORTED_POV_ARMS = true;
const FREE_FLY_MOUSE_SENSITIVITY = 0.0022;
...
function resizeStage(host: HTMLDivElement, stage: ThreeStage) {
...
function isFreeFlyKey(code: string) {
  return (
    code === "KeyW" ||
    code === "KeyA" ||
    code === "KeyS" ||
...
function syncFreeFlyAnglesFromCamera(stage: ThreeStage) {
...
function applyFreeFlyCameraRotation(stage: ThreeStage) {
  stage.camera.rotation.order = "YXZ";
  stage.camera.rotation.set(stage.freeFly.pitch, stage.freeFly.yaw, 0);
}

function updateFreeFlyCamera(stage: ThreeStage) {
...
function frameCameraAroundMap(stage: ThreeStage, mapScene: Group) {
...
function prepareMapSceneForReview(mapScene: Group, renderer: WebGLRenderer) {
...
function collectCameraCollisionMeshes(mapScene: Group) {
  const meshes: Mesh[] = [];
...
function replayViewForwardVector(yawRadians: number, pitchRadians: number) {
...
function disposeObjectMeshes(root: Object3D, options: { disposeSharedModelResources?: boolean } = {}) {
